 ...
 */
export const loadSingleJavascript = async (options = {}) => {
  const {
...
export const loadMultipleJavaScript = async ({ options, onLoad, onError }) => {
  if (!options) {
  ...
  return await Promise.allSettled(options.map(async item => await loadSingleJavascript(item)))
